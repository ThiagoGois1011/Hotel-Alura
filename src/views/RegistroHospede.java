...
public class RegistroHospede extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegistroHospede frame = new RegistroHospede(connection ,idReserva);
	 ...
	 * Create the frame.
	 */
	public RegistroHospede(Connection c, int idReserva) {
		RegistroHospede.connection = c;
		RegistroHospede.idReserva = idReserva;
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					Calendar d = txtDataN.getCalendar();
					String dia = "";
					String mes = "";
					try {
						 dia = String.valueOf(d.get(Calendar.DAY_OF_MONTH));
						 mes = String.valueOf(d.get(Calendar.MONTH) + 1);
					} catch (NullPointerException e2) {
						
						JOptionPane.showMessageDialog(null, "Deve preencher o campo 'Data de nascimento' da maneira correta.");
					}
					...
					Hospede h = new Hospede(txtNome.getText(),txtSobrenome.getText() ,data,txtNacionalidade.getSelectedItem().toString() ,
							txtTelefone.getText() ,RegistroHospede.idReserva );
					HospedeDAO dao = new HospedeDAO(connection);
					dao.salvar(h);
					String pattern = "\\D+";
					Pattern p = Pattern.compile(pattern);
					Matcher m = p.matcher(txtTelefone.getText());
					
					if(m.find()) {
			
						 throw new Exception("tem uma letra");
					...
					if(w.getMessage() == "tem uma letra") {
						JOptionPane.showMessageDialog(null, "O campo data deve conter apenas numeros.ex: 99999999999");
					}
