...
import funcionalidades.ValorReserva;
...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtValor = new JTextField(); //criar variavel string como nulo e reatribuir o valor no void comentado acima
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null && ReservasView.txtDataS.getCalendar().get(Calendar.DAY_OF_YEAR) >= ReservasView.txtDataE.getCalendar().get(Calendar.DAY_OF_YEAR) ) {		
					
					
					Calendar d = txtDataE.getCalendar();
					Calendar d1 = txtDataS.getCalendar();
					String dia = String.valueOf(d.get(Calendar.DAY_OF_MONTH));
					String mes = String.valueOf(d.get(Calendar.MONTH) + 1);
					String dia1 = String.valueOf(d1.get(Calendar.DAY_OF_MONTH));
					String mes1 = String.valueOf(d1.get(Calendar.MONTH) + 1);
					...
					String data = String.valueOf(dia + "/" + mes + "/" + d.get(Calendar.YEAR));
					String data1 = String.valueOf(dia1 + "/" + mes1 + "/" + d1.get(Calendar.YEAR));
					ReservaController r = new ReservaController();
					Reserva reserva = new Reserva(data, data1, Integer.valueOf(txtValor.getText()), String.valueOf(txtFormaPagamento.getSelectedItem()));
					r.salvar(reserva);
					RegistroHospede registro = new RegistroHospede(r.getConnection(), reserva.getId());
					registro.setVisible(true);
				} else {
					try {
						if(ReservasView.txtDataS.getCalendar().get(Calendar.DAY_OF_YEAR) < ReservasView.txtDataE.getCalendar().get(Calendar.DAY_OF_YEAR)) {
							JOptionPane.showMessageDialog(null, "A data de saída não pode ser antes da data de entrada.");
							return;
						}
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				Calendar data = txtDataE.getCalendar();
				Calendar data1 = txtDataS.getCalendar();
				List<Integer> dataEntrada = new ArrayList<Integer>();
				List<Integer> dataSaida = new ArrayList<Integer>();
				
				
				try {
					dataEntrada.add(data.get(Calendar.YEAR));
					dataEntrada.add(data.get(Calendar.MONTH));
					dataEntrada.add(data.get(Calendar.DAY_OF_MONTH));
					
					dataSaida.add(data1.get(Calendar.YEAR));
					dataSaida.add(data1.get(Calendar.MONTH));
					dataSaida.add(data1.get(Calendar.DAY_OF_MONTH));
					
					ValorReserva v = new ValorReserva(dataEntrada, dataSaida);
					int calculo = v.Calcula();
					if(calculo < 0) {
						JOptionPane.showMessageDialog(null, "A data de saída não pode ser antes da data de entrada.");
						return;
					}
					String valor = Integer.toString(calculo);
